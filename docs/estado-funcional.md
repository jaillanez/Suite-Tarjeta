...
| comercios | **Implementada** | Adhesión (máquina de estados), sucursales con PostGIS, usuarios y roles, PIN de cajero atado a dispositivo, turnos, QR de comprobante en PDF. **Precarga (§13.3):** comando idempotente `cargar_comercios` siembra comercios de prueba (sintéticos) ACTIVA con promos; bandera `precarga` + `baja_precarga` para baja en bloque (`docs/precarga-comercios.md`). |
