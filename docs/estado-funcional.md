...
| comercios | **Implementada** | Adhesión (máquina de estados), sucursales con PostGIS, usuarios y roles, PIN de cajero atado a dispositivo, turnos, QR de comprobante en PDF. **Precarga (§13.3/§14.2):** comando idempotente `cargar_comercios` siembra 36 comercios **reales de Rivadavia** (relevados de OpenStreetMap: nombre, rubro y coordenadas reales; **promos estimadas y marcadas**) en estado ACTIVA; bandera `precarga` + `baja_precarga` para baja en bloque (`docs/precarga-comercios.md`). |
